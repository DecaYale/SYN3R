...
from PGSR.utils.trainer import init_GSTrainer 
import sys 
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([latent_model_input, image_latents], dim=2) # NOTE: concat along channel dim?
...
def svd_render(image_o,masks, cond_image,image_path,warped_img_path,output_path,lambda_ts,weight_clamp, num_frames):
    pipe = StableVideoDiffusionPipeline.from_pretrained("stabilityai/stable-video-diffusion-img2vid-xt", torch_dtype=torch.float16, variant="fp16")
    pipe.to("cuda")
...
def save_colmap_format(extrinsics, intrinsics, image, depth, save_dir): 
    ...
    #debug
    # camera_dict_read = read_intrinsics_binary(os.path.join(save_dir,'cameras.bin'))
    # image_dict_read = read_extrinsics_binary(os.path.join(save_dir,'images.bin'))
    # import hashlib
    # def hash_dict(d):
    #     return hashlib.sha1(json.dumps(d, sort_keys=True)).hexdigest()
    # assert hash_dict(camera_dict_read) == hash_dict(camera_dict)
    # assert hash_dict(image_dict_read) == hash_dict(image_dict)
...
if __name__ == "__main__":
    ...
    #initialization
    image_o,masks, cond_image, poses_w2c, intrinsics = save_warped_image(save_path,args.image_path,depth_path,num_frames,args.degrees_per_frame,args.major_radius,args.minor_radius,args.inverse)
    ...
    # for iter in range(1):
        ...
        if iter==0:
            ...
            diffused_frames = svd_render(image_o,masks, cond_image,args.image_path,save_path,output_path,lambda_ts,args.weight_clamp, num_frames=num_frames)
