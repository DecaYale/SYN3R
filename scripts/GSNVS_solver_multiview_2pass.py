...
class StableVideoDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        image_embeddinge = self._encode_image(temp_cond[-1], device, num_videos_per_prompt, do_classifier_free_guidance) #by dy
        ...
        image_end = self.image_processor.preprocess(temp_cond[-1:], height=height, width=width) #by dy
        ...
        def image_to_image_latents(image, noise, dtype=image_embeddinge.dtype):
        ...
        # image = image + noise_aug_strength * noise # Note: maybe we should remove the noise?
        # image_end = image_end + noise_aug_strength * noise # Note: maybe we should remove the noise?
        ...
        weight_fw = torch.linspace(1,0,num_frames)[None,:,None,None,None].to(device=device, dtype=latents.dtype)
        # weight_fw = torch.sigmoid( torch.linspace(10,-10,num_frames)[None,:,None,None,None].to(device=device, dtype=latents.dtype)) 
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if 1: #i%2==0:
                    ...
                    for pass_i in ['forward', 'backward']: 
                    # for pass_i in ['backward', 'forward']: 
                        if pass_i == 'forward':
                            # continue
                            image_embedding__ = image_embeddings
                            temp_cond_latents__ = temp_cond_latents
                            mask__ = mask
                            image_latents__ = image_latent_start
                            latents__ = latents
                            lambda_ts__ = lambda_ts
                        else:
                            # continue
                            image_embedding__ = image_embeddinge
                            temp_cond_latents__ = temp_cond_latents.flip(dims=[1])#[:,::-1] 
                            mask__ = mask.flip(dims=[1]) #[:,::-1]
                            image_latents__ = image_latent_end
                            latents__ = latents.flip(dims=[1]) #[:,::-1] #????
                            # import pdb; pdb.set_trace()
                            lambda_ts__ = lambda_ts.flip(dims=[1]) #[:,::-1] # [100,25]



                        # latent_model_input = torch.cat([latents__] * 2) if do_classifier_free_guidance else latents__
                        # latent_model_input = self.scheduler.scale_model_input(latent_model_input, t,step_i=i)

                        # latent_model_input = torch.cat([latent_model_input, image_latents__], dim=2) # NOTE: concat along channel dim? image_latents is the condition?
                        latent_model_input_new = torch.cat([latent_model_input, image_latents__], dim=2) # NOTE: concat along channel dim? image_latents is the condition? # 
                        if pass_i == 'backward':
                            latent_model_input_new = latent_model_input_new.flip(dims=[1])

                        noise_pred = self.unet(
                            # latent_model_input,
                            latent_model_input_new,
                            t,
                            # encoder_hidden_states=image_embeddings,
                            encoder_hidden_states=image_embedding__,
                        ...
                        # latents = self.scheduler.step_multi_dgs(noise_pred, t, latents,temp_cond_latents,mask,lambda_ts,step_i=i).prev_sample #no big difference from step_single_dgs, the lambda_ts controls the merging ratio of the ref image and cond image.  #NOTE:!!!!!
                        # latents = self.scheduler.step_multi_dgs(noise_pred, t, latents__, temp_cond_latents__, mask__, lambda_ts__,step_i=i).prev_sample #no big difference from step_single_dgs, the lambda_ts controls the merging ratio of the ref image and cond image.  #NOTE:!!!!!
                        next_latents = self.scheduler.step_multi_dgs(noise_pred, t, latents__, temp_cond_latents__, mask__, lambda_ts__,step_i=i).prev_sample #no big difference from step_single_dgs, the lambda_ts controls the merging ratio of the ref image and cond image.  #NOTE:!!!!!
                        ...
                        # if pass_i == 'backward':
                        #     latents = latents.flip(dims=[1]) #[:,::-1] 
                        # latents_fb.append(latents)
                        latents_fb.append(next_latents)
                    ...
                    # import pdb; pdb.set_trace()
                    # latents = latents_fb[0]  # flip the backward latents to make it consistent with the forward latents   
                    # latents = latents_fb[1].flip(dims=[1])  # flip the backward latents to make it consistent with the forward latents   
                    ...
                    # latents = self.scheduler.undo_step(latents, i, ratio=0.3)
                else:
                    continue
                    step_id_last = i-1
                    latents = self.scheduler.undo_step(latents, step_id_last, ratio=0.5)

        # latents = latents.flip(dims=[1]) #  TODO:!!!!remove this line
...
def pose_interpolation(pose_start, pose_end):
    '''
...
def look_at_matrix(camera_position, target, up):
    ...
    view_matrix = rotation.T @ translation # NOTE: w2c?
...
# def save_warped_image(save_path,image_path,depth_path, # params for image and depth input
#                       num_frames, degrees_per_frame,major_radius,minor_radius,inverse=False, # params for camera pose generation
#                       intrinsics_for_GS=None, # params for GS
#                       ):
#     # num_frames = 25
...
#     image_o = image_o.resize((new_width, new_height),PIL.Image.Resampling.NEAREST) 
#     image = np.array(image_o)
#     if depth_path.endswith('.npy'):
#         depth = np.load(depth_path).astype(np.float32)
#     elif depth_path.endswith('.npz'):
#         depth = np.load(depth_path)['arr_0'].astype(np.float32)
#     else:
#         raise NotImplementedError
...
#     if intrinsics_for_GS is None:
#         print("No intrinsics are provided for GS. Using default intrinsics")
#         focal = 260.
#         K = np.eye(3)
#         K[0,0] = focal; K[1,1] = focal; K[0,2] = 1024./2; K[1,2] = 576./2
#     else:
#         K = intrinsics_for_GS
#         K[0,:] = K[0,:]*new_width/original_width
#         K[1,:] = K[1,:]*new_height/original_height
...
#     #
#     save_colmap_format(extrinsics=poses, intrinsics=K, image=image, depth=depth, save_dir=os.path.join(save_path, '../', 'sparse') )

#     return image_o,masks,cond_image, poses, K #w2c

def svd_render(image_l, image_r, masks, cond_image,output_path,lambda_ts, num_frames=25, save_prefix=''):
    pipe = StableVideoDiffusionPipeline.from_pretrained("stabilityai/stable-video-diffusion-img2vid-xt", torch_dtype=torch.float16, variant="fp16")
    pipe.to("cuda")

    cv2.imwrite(os.path.join(output_path, "image_r.png"), image_r*255)

    # frames = pipe([image_l],temp_cond=cond_image+[image_r],mask = masks,lambda_ts=lambda_ts,num_frames=num_frames, decode_chunk_size=8, num_inference_steps=100).frames[0] 
    frames = pipe([image_l],temp_cond=cond_image+[image_r],mask = masks,lambda_ts=lambda_ts,num_frames=num_frames, decode_chunk_size=8,num_inference_steps=30).frames[0] 
...
def svd_render_single(image_o, masks, cond_image,image_path,warped_img_path,output_path,lambda_ts, num_frames=25):
    ...
    # frames = pipe([image_o],temp_cond=cond_image,mask = masks,lambda_ts=lambda_ts,weight_clamp=weight_clamp,num_frames=25, decode_chunk_size=8,num_inference_steps=100).frames[0]
    frames = pipe([image_o],temp_cond=cond_image,mask = masks,lambda_ts=lambda_ts,num_frames=num_frames, decode_chunk_size=8,num_inference_steps=100).frames[0]
    # frames = pipe([image_o],temp_cond=cond_image,mask = masks,lambda_ts=lambda_ts,weight_clamp=weight_clamp,num_frames=num_frames, decode_chunk_size=8,num_inference_steps=10).frames[0]
...
def compute_dists(interpolated_poses):
...
def search_hypers(pose,dists,sigmas,save_path):
    sigmas = sigmas[:-1]
...
def search_hypers_single(sigmas,save_path):
    sigmas = sigmas[:-1]
    sigmas_max = max(sigmas)
...
# def warp_images(interpolated_poses,src1,src2,save_path):
def warp_images(intrinsics, interpolated_poses, image_l, image_r, depth_l, depth_r, save_path, save_file_name_prefix=''):
    '''
    ...
    # masks_l =[]
    # from pathlib import Path
    # img_path = Path(save_path).parent
    new_width, new_height = 1024,576  # New dimensions

    pose_s_l = interpolated_poses[0]
    # image_l = PIL.Image.open(os.path.join(img_path,'000001.jpg'))
    # image_l = image_l.resize((1024,576),PIL.Image.Resampling.NEAREST) 
    ...
    # if os.path.exists(os.path.join(img_path,'000001.npy')):
    ...
    # if os.path.exists(os.path.join(img_path,'000002.npy')):
    #     depth_r = np.load(os.path.join(img_path,'000002.npy'))
    # elif os.path.exists(os.path.join(img_path,'000002.pfm')):
    #     depth_r,_= read_pfm(os.path.join(img_path,'000002.pfm'))
    # new_width, new_height = 1024,576  # New dimensions
...
def save_colmap_format(extrinsics, intrinsics, image, depth, save_dir): 
    '''extrinsics: 4x4 matrices, w2c 
    ...
    from solver_utils.colmap_loader import Image, Camera, rotmat2qvec, write_extrinsics_binary, write_intrinsics_binary, read_extrinsics_binary, read_intrinsics_binary, write_points3D_binary, read_points3D_binary
    import open3d as o3d 
    pose0 = extrinsics[0] # w2c
    # pose0_c2w = np.linalg.inv(pose0) #c2w

    # new_w2c_list = []
    image_dict={}
    for i,pose in enumerate(extrinsics):
        # pose_c2w = np.linalg.inv(pose) #c2w
        new_w2c = pose@np.linalg.inv(pose0) # taking the first pose as the world coordinate
        ...
        image_dict[image_id] = Image(
                        ...
                        camera_id=int(0), name=image_name,
    ...
    camera_dict= { 
        camera_id:  Camera(id=camera_id,
                    model="PINHOLE",
                    width=width,
                    height=height,
                    params=np.array([intrinsics[0,0], intrinsics[1,1], intrinsics[0,2], intrinsics[1,2]]))
        }

    os.makedirs(save_dir, exist_ok=True)
    write_intrinsics_binary(os.path.join(save_dir,'cameras.bin'), camera_dict)
    write_extrinsics_binary(os.path.join(save_dir,'images.bin'), image_dict)
    ...
    write_points3D_binary(f'{save_dir}/points3D.bin', dataframes)
    xyzs, rgbs, errors = read_points3D_binary(f'{save_dir}/points3D.bin')
...
class DiffusionGS:
    def __init__(self, GSTrainer:GSTrainer, num_input_views=12, save_dir=None, debug=False):
        ...
        self.intrinsics, self.extrinsics = gsTrainer.scene.getTrainCameras()[0].get_calib_matrix_nerf() # K, w2c 
        pass 
    ...
    def render_GS(self, idx):
        cam = gsTrainer.scene.getTrainCameras()[idx]
        ...
        render_res = gsTrainer.render_view(cam)
        depth = render_res['plane_depth'].detach().squeeze().cpu().numpy()
        return pose, image, depth

    def densify_views(self, down_sample_rate=1):
    ...
    def _interpolate_between(self, idx1, idx2, replace=True):
        ...
        interpolated_poses = pose_interpolation(pose1,pose2)

        dists, min_indice = compute_dists(interpolated_poses)

        image_o,image_o2,masks, cond_image = warp_images(self.intrinsics, interpolated_poses, image_l=image1, image_r=image2, depth_l=depth1, depth_r=depth2, save_path=save_path, save_file_name_prefix=f'{idx1}') 

        sigma_list = np.load('sigmas/sigmas_100.npy').tolist()
        lambda_ts = search_hypers(interpolated_poses,dists,sigma_list,save_path)

        diffused_frames = svd_render(image_o, image_o2, masks, cond_image, output_path, lambda_ts, num_frames=num_frames, save_prefix=f'{idx1}_') #NOTE:!!!!
        ...
        diffused_frames = [torch.from_numpy(np.asarray(fr)).permute([2,0,1]).cuda()/255. for fr in diffused_frames]   # HWC->CHW

        # import pdb; pdb.set_trace() 
        if replace:
            diffused_frames[0] = torch.from_numpy(np.asarray(image_o)).permute([2,0,1]).cuda() 
            diffused_frames[-1] = torch.from_numpy(np.asarray(image_o2)).permute([2,0,1]).cuda() 

        return diffused_frames, interpolated_poses
    

    def refine_GS(self, dense_views, dense_poses, intrinsics, confidence=None, gs_start_iter=0,  ):
        # refine GS with dense views
        self.gsTrainer.update_cameras(dense_views, dense_poses, intrinsics)
    ...
    def run(self):
        ...
        dense_views, dense_poses = self.densify_views(down_sample_rate=1)

        self.refine_GS(dense_views, dense_poses, intrinsics=self.intrinsics, confidence=None)
...
if __name__ == "__main__":
    ...
    # save_path = os.path.join(args.folder_path, args.iteration)
    ...
    if not os.path.exists(depth_path):
        depth_path = os.path.join(args.folder_path,'depth',image_name+'.npz')
    ...
    # new_width, new_height = 1024,576  # New dimensions
    # image_o = cv2.imresize(image_o, (new_width, new_height))
    # masks = F.interpolate(masks[None, None], size=(new_height, new_width), mode='nearest', align_corners=True).squeeze(0).squeeze(0)
    # cond_image = [fr.resize((new_width, new_height),PIL.Image.Resampling.NEAREST)  for fr in cond_image]
    ...
    runner = DiffusionGS(gsTrainer, num_input_views=args.num_train_samples, save_dir=None, debug=True )
    runner.run()
    ...
    # #TODO: hard code here
    K = np.array([1725.6083768722879,0,1920, 
                  0,1724.4312361289274,1080,
                  0,0,1]).reshape([3,3])
    K[:2,:] = K[:2,:]/4 
    # K = None
    #initialization
    cam_start, cam_end = gsTrainer.scene.getTrainCameras()[0], gsTrainer.scene.getTrainCameras()[1]
    pose_start, pose_end =cam_start.world_view_transform.transpose(0,1).cpu().numpy(), cam_end.world_view_transform.transpose(0,1).cpu().numpy() # world to cam? transpose to make right multiplication->left multiplication
    image_start, image_end = cam_start.get_image()[0].permute([1,2,0]).cpu().numpy()*255, cam_end.get_image()[0].permute([1,2,0]).cpu().numpy()*255 # HWC

    render_start, render_end = gsTrainer.render_view(cam_start), gsTrainer.render_view(cam_end)
    depth_start, depth_end = render_start['plane_depth'].detach().squeeze().cpu().numpy(), render_end['plane_depth'].detach().squeeze().cpu().numpy()
    ...
    src1, src2= 0, 1 #TODO: hard code here
    
    # save_path = f'example_imgs/multi/{args.scene}/{args.scene}_src{src1}_{src2}/'
    # if not os.path.exists(save_path):
    #     os.makedirs(save_path)

    # image_o,masks, cond_image, poses_w2c, intrinsics = save_warped_image(save_path,args.image_path,depth_path,num_frames,args.degrees_per_frame,args.major_radius,args.minor_radius,args.inverse, intrinsics_for_GS=K)
    image_o,image_o2,masks, cond_image = warp_images(K, interpolated_poses, image_l=image_start, image_r=image_end, depth_l=depth_start, depth_r=depth_end, save_path=save_path) # the cond image is warpped half by half based on the the distance to the two ends 



    # gt_images = [image_o]+cond_image
    ...
    for iter in range(1):
        ...
        if iter==0:
            ...
            # lambda_ts = search_hypers_single(sigma_list,args.folder_path) #NOTE:!!!!
            lambda_ts = search_hypers(interpolated_poses,dists,sigma_list,save_path)

            # cond_image.append(image_o2)
            # masks = torch.cat([masks,masks[-1:]], dim=0 )
            # masks = torch.zeros_like(masks) # NOTE: !!!!
            # diffused_frames = svd_render_single(image_o, masks, cond_image,args.image_path,save_path,output_path,lambda_ts, num_frames=num_frames) #NOTE:!!!!
            diffused_frames = svd_render(image_o, image_o2, masks, cond_image, output_path, lambda_ts, num_frames=num_frames) #NOTE:!!!!
            
            # diffused_frames = svd_render(image_o2, image_o, masks.flip(dims=[0]), cond_image[::-1], output_path,lambda_ts.flip(dims=[1]), num_frames=num_frames) #NOTE:!!!!

            diffused_frames = [torch.from_numpy(np.asarray(fr)).permute([2,0,1]).cuda()/255. for fr in diffused_frames]    
        else:
            # sigma_list = np.load('sigmas/sigmas_100.npy').tolist()
            # lambda_ts = search_hypers(sigma_list,args.folder_path)

            masks = torch.ones_like(masks)
            cond_image = gsTrainer.rgb_rendering[1:]
            diffused_frames = svd_render(image_o,masks, cond_image,args.image_path,save_path,output_path,lambda_ts, num_frames=num_frames)
            diffused_frames = [torch.from_numpy(np.asarray(fr)).permute([2,0,1]).cuda()/255. for fr in diffused_frames]    
            # import pdb;pdb.set_trace()

        # try:
        #     if gsTrainer is None:
        #         gsTrainer = init_GSTrainer(args, lp, op, pp, shuffle_cameras=False)  # intialize the GS scene 
        #         gs_start_iter = 0
        #         epoch_iteration = gsTrainer.opt.iterations
        # except:
        ...
        break
        gsTrainer.set_gt_image(diffused_frames)

        
        gsTrainer.training(gs_start_iter, epoch_indicator=iter)
        gs_start_iter = gsTrainer.opt.iterations + 1 
        gsTrainer.opt.iterations = gs_start_iter+epoch_iteration # set the end iteration for next epoch
