...
#added by dy
def write_points3D_binary(file_path, points):
    """
...
def write_extrinsics_binary(path_to_model_file, images):
    ...
    with open(path_to_model_file, "wb") as fid:
        ...
        for image_id, image in images.items():
            fid.write(struct.pack("=idddddddi", image_id, *image.qvec, *image.tvec, image.camera_id))
            ...
            for xy, point3D_id in zip(image.xys, image.point3D_ids):
                fid.write(struct.pack("=ddq", *xy, point3D_id))
...
#added by dy
def write_intrinsics_binary(path_to_model_file, cameras):
    """
    see: src/base/reconstruction.cc
        void Reconstruction::WriteCamerasBinary(const std::string& path)
        void Reconstruction::ReadCamerasBinary(const std::string& path)
    ...
    with open(path_to_model_file, "wb") as fid:
        ...
        for camera_id, camera in cameras.items():
            fid.write(struct.pack("=iiQQ", camera_id,
